import {api__get__posts,api__post__posts,api__patch__posts,api__delete__posts,api__get__one__post,api__get__my__post,api__get__verified__posts,api__get__admin__posts} from '../API/api__posts.js';
...
export const action__delete__posts = (id) => async (dispatch) => {
 ...
 export const action__get__admin__posts=(id)=> async(dispatch)=>{ 
  try {
      const {data}=await api__get__admin__posts(id);
      dispatch({ type: 'FETCH ADMIN', payload:data});
  } catch (error) {
      console.log(error);
  }
